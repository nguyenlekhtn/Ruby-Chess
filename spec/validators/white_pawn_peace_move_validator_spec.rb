...
describe WhitePawnPeaceMoveValidator do
  let(:board) { instance_double(Board) }
  subject(:validator) { described_class.new(board) }

  context 'when WhitePawnPeaceMoveValidator is a child of Validator' do
  ...
  describe '#valid?' do
    ...
    context 'when end_pos is empty' do
      ...
      context 'when end_pos is at (1, 0) from start_pos' do
      ...
      context 'when end_pos is at (2, 0) from start_pos' do
        context 'when start_pos is not at  row 1' do
        ...
        context 'when start_pos is at row 1' do
          let(:cell1) { Cell.new(1, 2) }
